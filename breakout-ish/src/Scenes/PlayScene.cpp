...
PlayScene::PlayScene(const CoreComponents& components, sf::Sprite paddle)
    : m_Components(components), m_Player(std::make_unique<Paddle>(paddle)), m_PaddleSpeed(1500.f)
{
}
...
void PlayScene::processInput()
{
    ...
    {
        m_Player->move(m_PaddleSpeed * m_Components.m_TimePerFrame->asSeconds() * -1.f, 0.f);
    }
    ...
    {
        m_Player->move(m_PaddleSpeed * m_Components.m_TimePerFrame->asSeconds(), 0.f);
    }
}
...
{
    m_Components.m_RenderWindow->draw(*m_Player);
}

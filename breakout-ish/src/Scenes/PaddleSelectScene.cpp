...
void PaddleSelectScene::initialize()
{
    ...
    m_Components.m_AssetManager->loadTexture(
        "paddles-long", "assets/spritesheets/paddles-long.png");
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        {
            m_Paddles.push_back(sf::Sprite(
                m_Components.m_AssetManager->getTexture("paddles-long"),
        ...
        m_Paddles[i].setPosition(m_Components.m_RenderWindow->getSize().x / 2.f,
                                 m_Components.m_RenderWindow->getSize().y /
                                     1.5f);
    }
...
void PaddleSelectScene::processInput()
{
    ...
    else if (m_Components.m_InputManager->isKeyPressed(sf::Keyboard::Enter))
    {
        ...
        m_Components.m_SceneManager->pushScene(
            ScenePtr(std::make_unique<PlayScene>(
                m_Components, m_Paddles[m_SelectedPaddleIndex])),
            true);
    }
}

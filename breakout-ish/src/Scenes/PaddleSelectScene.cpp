...
{
    m_Components.m_AssetManager->loadTexture("paddles", "spritesheets/paddles.png");
    m_Components.m_AssetManager->loadTexture("paddles-long", "spritesheets/paddles-long.png");
    m_Components.m_AssetManager->loadSound("select", "sounds/select.wav");
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        m_Paddles[i].setOrigin(m_Paddles[i].getLocalBounds().width / 2.0f,
                               m_Paddles[i].getLocalBounds().height / 2.0f);
        m_Paddles[i].setPosition(m_Components.m_RenderWindow->getSize().x / 2.0f,
...
void PaddleSelectScene::processInput()
{
    if (m_Components.m_InputManager->isKeyPressed(sf::Keyboard::Left))
    {
        ...
        {
            m_SelectedPaddleIndex = static_cast<unsigned int>(m_Paddles.size() - 1);
        }
    }
